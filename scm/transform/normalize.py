...
def normalize_moves(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 필수 컬럼 매핑 (한글 컬럼명 포함)
    column_mapping = {
        "resource_code": ["resource_code", "sku", "product_code", "item_code", "SKU", "Product Code", "품번"],
        "qty_ea": ["qty_ea", "quantity", "qty", "수량", "Quantity", "Qty"],
        "from_center": ["from_center", "from_warehouse", "from_location", "출발센터", "출발창고"],
        "to_center": ["to_center", "to_warehouse", "to_location", "도착센터", "목적창고"],
        ...
        "inbound_date": ["inbound_date", "received_date", "입고일", "입고예정일", "Inbound Date"],
        "carrier_mode": ["carrier_mode", "transport_mode", "운송수단", "운송사", "Carrier Mode"]
    }
    ...
    # 센터 정보가 이미 있으면 사용, 없으면 기본값 설정
    if "from_center" not in result.columns:
        result["from_center"] = "Unknown"
    if "to_center" not in result.columns:
